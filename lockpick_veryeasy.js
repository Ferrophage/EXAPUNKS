...
function getTitle(){return "X-PEL 500 Lockpicking (Very Easy)";};function getSubtitle(){return "by Ferrophage";};
function getDescription(){
    return "This is a very basic lock that needs to be picked. Write a three digit code to #LOCK to input a code. Reading from #LOCK will return a three digit number.\n"
...
function makeHostsandLinks(){
    ...
    lockHost = createHost("x-pel 500", 5, 1, 3, 1);
...
function lockOutputFunc(){
    ...
    for (i in guessDigits){
        if (guessDigits[i] == keyDigits[i]){
         output += Math.floor(Math.pow(10,2-i))
        }
    }
    if (output == 111 && ACCESS_OPENED == false ){
        modifyLink(lockHost_to_secureHost,800,-1)
        ACCESS_OPENED = true
        setCustomGoalCompleted(customGoal_1)
        CUSTOM_1_COMPLETE = true
    }else if (output != 111 && CUSTOM_1_COMPLETE == true){
